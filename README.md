...
A desktop-first Bangla phonetic keyboard inspired by the classic Avro control bar. It has a Flutter configuration window, tray/window integration, a layout viewer, candidate preview, persisted preferences, and a native platform boundary for system-wide input and Unicode output.
...
Use the Bangla pill in the top bar to switch languages. The documented system-wide shortcut is `Ctrl + Alt + B`; a production release should register that shortcut natively and expose a shortcut editor.
...
- Add a tray click/menu policy and hide-to-tray close behavior for release builds.
